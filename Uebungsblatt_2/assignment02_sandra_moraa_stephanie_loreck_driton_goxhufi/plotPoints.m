function plotPoints(points, edges, w, h)
...
for a=1:length(points)
    if points(3,a) > 0
        plot(points(1,a), points(2,a), '*');
    end
end
for a=1:length(edges)
    if points(3, edges(a,1)) > 0 || points(3, edges(a,2)) > 0
        line([points(1, edges(a,1)) points(1, edges(a,2))],[points(2, edges(a,1)) points(2, edges(a,2))], 'Color', 'red');
    end
end
hold on;
end
